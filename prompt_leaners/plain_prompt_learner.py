...
class PlainPromptLearner(nn.Module):
    ...
    def create_sentence_embeds_template(self, clip_model, num_ranks, num_classnames, psudo_sentence_tokens):
        with torch.no_grad():
        ...
        sentence_embeds = null_embed[None, None].repeat(
            num_ranks * num_classnames, self.clip_max_num_tokens, 1
        )  # not the same null_embed!
        argmax_index = psudo_sentence_tokens.argmax(dim=-1)
        index = torch.arange(num_ranks * num_classnames)

        sentence_embeds[:, 0, :] = sot_embed
        sentence_embeds[index, argmax_index] = eot_embed
        sentence_embeds[index, argmax_index - 1] = full_stop_embed
